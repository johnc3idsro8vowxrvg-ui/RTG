...
为港口轮胎式集装箱门式起重机（RTG）构建面向大车防撞的 BEV 感知系统。V1 推理闭环使用集卡侧 L1/L2 两个 OS1 128 线雷达拼接点云，通过 CenterPoint LiDAR-only 输出 3D/BEV 障碍物检测结果和分级碰撞风险预警；4 相机 + 4 雷达在 V1 中用于录制和可视化，禁行侧与相机融合为后续扩展。
...
| 文档 | 说明 |
|---|---|
| [项目参数基线](doc/项目参数基线.md) | **权威口径** — 坐标系、传感器、V1边界、标定、预警、性能统计 |
| [项目完整报告](doc/项目完整报告.md) | **综合报告** — 汇总全部项目文档，面向评审/交付阅读 |
...
- **calib_from_bag.yaml** — Bag 估计标定（2026-06-11，提交 git）
- **geometry.yaml** — RTG 几何与车道（默认设计值，提交 git）
- **warning.yaml** — 预警参数（算法配置，支持热更新，提交 git）
- **system.yaml** — 系统/部署（默认开发环境，提交 git）
...
| TC-06 | 运动状态推断 → 方向正确切换 |
| TC-07 | 静止目标 → 不误报 |
| TC-08 | 系统性能 → 分阶段延迟 P50/P95, 帧率≥10Hz |
